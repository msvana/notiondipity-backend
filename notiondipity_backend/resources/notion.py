...
async def get_page_text(page_id: str, access_token: str) -> str:
    url = os.path.join(config.NOTION_BASE_URL, 'blocks', page_id, 'children')

    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=_create_headers_from_token(access_token)) as response:
            if response.status != 200:
                raise IOError(f'Notion API returned status code {response.status}')
            full_text = []
            for block in (await response.json())['results']:
