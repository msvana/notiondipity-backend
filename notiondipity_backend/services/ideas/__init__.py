...
class IdeaService:
    ...
    def __init__(self, connection: Connection, openai_client: AsyncOpenAI):
        self._openai_client = openai_client
        self._idea_cache = IdeaCache(connection)
        self._comparison_service = ComparisonService(openai_client, connection)
    ...
    async def regenerate_and_store_ideas(self, user_id: str, pages: list[PageEmbeddingRecord]) -> list[Idea]:
        ...
        ideas = await generator.get_ideas(self._openai_client, self._comparison_service, pages, user_id)
    ...
    def unsave_idea(self, user_hash: str, idea_id: int):
        is_owner = self._idea_cache.get_owner_hash(idea_id) == user_hash
        if not is_owner:
            raise ValueError('User does not own the idea')
        self._idea_cache.unsave_idea(idea_id)
    ...
    def _cached_idea_to_idea(cached_idea: CachedIdea, user_id: str) -> Idea:
        ...
        title = cached_idea.get_title(user_id)
        description = cached_idea.get_description(user_id)
        if title is None or description is None:
            raise ValueError('Cached idea does not have a title or description')
        idea = Idea(title, description, cached_idea.idea_id, True, cached_idea.saved)
        return idea
